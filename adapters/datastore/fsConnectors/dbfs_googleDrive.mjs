// freezr.info - dbfs_googleDrive.mjs
// Google Drive file system connector (MODERNIZED TO ES6 MODULES)

// ============================= NOT TESTED YET 2025-10 =============================


// 
// Original: fs_obj_googleDrive.js 2021-01
// Note on Security:
...
function googleDriveFS (credentials = {}, options = {}) {
  // console.log('googleDriveFS  - new goog credentials to set ', { credentials })
  ...
  // // console.log('googleDriveFS - set 1  credentials - ', this.auth.credentials)
...
googleDriveFS.prototype.initFS = function (callback) {
  ...
  if (this.credentials.refreshToken) {
    // console.log('All good good on goog - have refreshToken')
    ...
    console.warn('googleDriveFS initFS - callback err - no tokens')
...
googleDriveFS.prototype.writeFile = function (path, contents, options, callback) {
  // console.log(' - goog writefile', path)
        ...
        if (err) {
          ...
          */
          ...
          self.drive.files.create({
            ...
            if (err) {
              // Handle error
              console.warn('goog writeFile', '1 - write err in goog writeFile', { file, err })
              callback(err)
            } else {
              // console.log('goog writeFile success - File all: ', (file.data ? file.data.id : 'not found'), file)
              callback(null)
            }
          })
        } else { // exists
          ...
          self.drive.files.update({
            ...
            if (err) {
              // Handle error
              console.warn('goog writeFile', '2 overwrite - write err in goog writeFile', err)
              callback(err)
            } else {
              // console.log('goog writeFile success overwrite - File all: ', (file.data ? file.data.id : 'not found'), file)
              callback(null)
            }
          })
        }
...
googleDriveFS.prototype.rename = function (fromPath, toPath, callback) {
  // console.log(' - goog-rename ', fromPath, toPath)
  const self = this

  self.fileOrFolderExistsOnGoog(fromPath, { fileOnly: true }, (err, exists, fileInfo) => {
    // console.log('fileOrFolderExistsOnGoog ', { fileInfo })
    ...
    if (err) {
      ...
      self.unlink(toPath, function (err, resp) {
        if (err) {
          ...
          // console.log('goog-rename - to copy ', { newName, parentId })
          self.drive.files.copy({ fileId: fromFileId, requestBody: { name: newName, parents: [parentId] } }, (err, results) => {
            // console.log('goog-rename - copied ', { newName, parentId, err, results })

            self.drive.files.delete({
              fileId: fromFileId
            }, function (err, results) {
              // if (err)
              if (err) console.warn('goog-rename delete  ', { err, results })
              callback(err)
            })
          })
        }
  ...
  self.fileOrFolderExistsOnGoog(path, { fileOnly: false }, (err, exists, fileInfo) => {
    if (err) {
      ...
      self.drive.files.delete({
        ...
        if (err) console.warn('goog delete err ', { err, results })
        callback(err)
      })
    }
  })
...
googleDriveFS.prototype.isPresent = function (file, callback) {
  // console.log(' - goog-isPresent ', { file })
  if (file.substring(0, 1) === '/') {
    file = file.substring(1)
  }
  this.fileOrFolderExistsOnGoog(file, { fileOnly: false }, function (err, exists, fileInfo) {
    // const folderId = fileInfo.fileId
    if (err && err.message === 'folder does not exist') {
      console.log('unsetting err')
      err = null
      exists = false
    }
    callback(err, exists)
...
googleDriveFS.prototype.exists = function (file, callback) {
  // console.log(' - goog-exists ', file)
  this.fileOrFolderExistsOnGoog(file, { fileOnly: false }, (err, exists, fileInfo) => {
    // const folderId = fileInfo.fileId
    callback((err || exists))
  })
}
googleDriveFS.prototype.stat = function (file, callback) {
  // console.log(' - goog-exists ', file)
  this.fileOrFolderExistsOnGoog(file, { fileOnly: false }, (err, exists, fileInfo) => {
    // const folderId = fileInfo.fileId
    if (!exists) {
...
googleDriveFS.prototype.mkdirp = function (path, callback) {
  return this.GetOrMakeFolders(path, { doNotMake: false, returnId: false }, function (err, ids) {
    callback(err, ids)
  })
}
googleDriveFS.prototype.readdir = function (dirpath, dummy, callback) {
  // Note current implementation doesnt return more files than the limit
  // console.log('goog reading dir ', dirpath)
  const self = this

  self.GetOrMakeFolders(dirpath, { doNotMake: true, returnId: true }, (err, folder) => {
    if (err) {
      console.warn('readdir', 'could not read', { dirpath, err })
      callback(err)
    } else if (!folder) {
      ...
      async.doWhilst(function (cb) {
        self.drive.files.list({
          ...
          if (err) {
            // Handle error
            console.warn('readdir', 'whilst', err)
            cb(err)
          } else if (!res.data || !res.data.files) {
            console.warn('readdir', res)
...
googleDriveFS.prototype.readFile = function (path, options, callback) {
  ...
  // console.log(' - goog-readFile ', path)
  ...
  self.fileOrFolderExistsOnGoog(path, { fileOnly: true }, (err, exists, fileInfo) => {
    if (err) {
      ...
      if (!fileInfo || !fileInfo.fileId) console.warn('readFile', 'snbh - fileInfo missing on readfile ', path)
      self.drive.files.get({
        ...
        if (err) console.warn('goog-readfile ', { err, results })
...
googleDriveFS.prototype.getFileToSend = function (path, options, callback) {
  // console.log(' - goog-getFileToSend ', path)
  const self = this
  self.fileOrFolderExistsOnGoog(path, { fileOnly: true }, (err, exists, fileInfo) => {
    if (err) {
      callback(err)
      ...
      if (!fileInfo || !fileInfo.fileId) console.warn('snbh - fileInfo missing on getFileToSend ', path)
      self.drive.files.get({
        fileId: fileInfo.fileId,
        alt: 'media'
      }, function (err, results) {
...
googleDriveFS.prototype.removeFolder = function (path, callback) {
  // console.log(' - goog-removeFolder ', path)
...
googleDriveFS.prototype.appendNedbTableFile = function (path, contents, encoding, callback) {
  ...
  // console.log(' - goog-appendNedbTableFile start ', path, contents) // new Date().toLocaleTimeString() + ' : ' + new Date().getMilliseconds()
...
googleDriveFS.prototype.readNedbTableFile = function (path, encoding, callback) {
  // read file goes through folder with appends, and adds them to content
  // console.log(' - goog-readNedbTableFile ', path)
  var self = this
  const [appendDirectory] = getnamesForAppendFilesFrom(path)
  let contents = ''
  ...
  self.readFile(path, {}, (err, mainfileContent) => {
    if (err && err.message !== FILE_DOES_NOT_EXIT) {
      console.warn('file read err message', err.message)
      callback(err)
    } else {
      ...
      self.getAllAppendDirectoryFiles(appendDirectory, true, (err, results) => { // { folderId, entries }
        if (!err && results && results.entries && results.entries.length > 0) {
          // console.log('todo - can one just do a search for all files and get the content with it, as opposed to getting list and then the content')
          async.forEach(results.entries, (entry, cb) => {
            self.drive.files.get({
              ...
              // if (err) console.warn('goog readNedbTableFile', { err, results })
              cb(err)
            })
          }, (err) => {
            if (err) {
              console.warn('Err in getting nedb file content', err)
              callback(err)
            } else {
              ...
              // console.log('e1 DB populated with:' + contents + '.END')
          ...
          // console.log('e2 DB populated with:' + contents + '.END')
...
googleDriveFS.prototype.writeNedbTableFile = function (filename, data, options, callback) {
  ...
  // console.log(' - goog- writeNedbTableFile ', { filename, data }) // data
  ...
  self.writeFile(filename, data, {}, function (err) {
    if (err) {
      console.warn('writeNedbTableFile', 'error writing file in writeNedbTableFile', err)
      return callback(err)
    } else {
      self.getAllAppendDirectoryFiles(appendDirectory, true, (err, results) => { // { folderId, entries }
        if (!err && results && results.entries && results.entries.length > 0) {
          ...
          async.forEach(results.entries, (entry, cb) => {
            ...
            if (err) console.warn('Err in deleting nedb appendDirectory files ', err)
            callback(err)
          })
        } else {
          callback(err)
...
googleDriveFS.prototype.deleteNedbTableFiles = function (file, callback) {
  // console.log('goog deleteNedbTableFiles ', file)
  const [appendDirectory] = getnamesForAppendFilesFrom(file)
  const self = this

  self.unlink(file, function (err) {
    if (err) {
      console.warn('err in deleteNedbTableFiles - todo: if filenotfound then ignore for file ' + file, err)
      return callback(err)
    } else {
      self.unlink(appendDirectory, function (err) {
        if (err) console.warn('err in deleteNedbTableFiles - todo: if filenotfound then ignore for file ' + file, err)
        return callback(err)
      })
    }
  })
...
googleDriveFS.prototype.crashSafeWriteNedbFile = function (filename, data, callback) {
  ...
  // console.log('goog crashSafeWriteNedbFile write ', { filename }) // data
        ...
        async.forEach(results.entries, (entry, cb2) => {
          if (timeFromPath(entry.name) < now) {
            self.drive.files.delete({
              ...
              if (err) console.warn('goog deleted appendDirectory in crashSafeWriteNedbFile file (remove console and move "cb" to delete)', { err, results })
              cb2(err)
            })
          } else {
            cb2(null)
  ...
  function (err) {
    if (err) console.warn('end of crashSafeWriteNedbFile write', { err, data })
    return callback(err)
  })
}
...
googleDriveFS.prototype.fileOrFolderExistsOnGoog = function (file, options, callback) {
  // console.log(' - goog-fileOrFolderExistsOnGoog ', file)

  options = options || {}
  const self = this
  var partPaths = file.split('/')
  const fileName = partPaths.pop()
  if (partPaths[0] === '') partPaths.shift()
  const path = partPaths.join('/')
  let returnValue = false

  this.GetOrMakeFolders(path, { doNotMake: true, returnId: true }, (err, folder) => {
    if (err) {
      console.warn('GetOrMakeFolders 1 - could not check folder ', folder)
      callback(err)
    } else if (!folder) {
      console.warn('GetOrMakeFolders !folder- could not check folder ', folder)
      callback(new Error('coudl not reach folder'))
    } else if (!folder.folderId) {
      console.warn('GetOrMakeFolders - !folder.folderId - could not check folder ', folder)
      ...
      // console.log('goog queryString is ' + queryString)

      self.drive.files.list({
        q: queryString,
        fields: 'nextPageToken, files(id, name, createdTime, modifiedTime, viewedByMeTime, kind, mimeType)', // , files/parents
        spaces: 'drive',
        pageToken: null
      }, function (err, response) {
        if (err) {
          // Handle error
          if (err) console.warn('Handle error - trying to list', { file, err })
          callback(err)
          ...
          returnValue = true
          callback(null, returnValue, { parentId: folder.folderId, fileId: response.data.files[0].id, other: response.data.files[0] })
        } else if (response && response.data && response.data.files && response.data.files.length > 1) {
          returnValue = true
          ...
          console.warn('in goog-fileOrFolderExistsOnGoog - invalid response ', { file, response })
...
googleDriveFS.prototype.removeFromPathIds = function (path) {
  ...
  // let foundEnd = false
  // console.log('removeFromPathIds len ', pathParts.length, { path, pathStruct })
...
googleDriveFS.prototype.GetOrMakeFolders = function (path, options, callback) {
  ...
  // console.log(' - goog-GetOrMakeFolders ', path)
  ...
  async.whilst(
        ...
        self.drive.files.list({
          ...
          if (err) {
            ...
            if (err && err.message && err.message.indexOf('No refresh token is set') > -1) {
              console.warn('auth error from goog ', { err })
              err = new Error('authorisation error')
            }
            cb(err)
          } else if (response && response.data && response.data.files && response.data.files.length >= 1) {
            if (response.data.files.length > 1) console.warn('dbfs_googleDrive GetOrMakeFolders for ' + path + ' - Got more than 1 folder - ignoring error for the moment')
            ...
            if (options.doNotMake) {
              ...
              // // console.log('found no files - will create ', response)
              var fileMetadata = {
                name: currentFolderName,
              ...
              // // console.log('folder doesnt exist - create it ', fileMetadata)
              self.drive.files.create({
                resource: fileMetadata,
                fields: 'id'
              }, function (err, theFile) {
                if (err) {
                  console.warn('Handle error in creating file' + path, err)
                  cb(err)
                } else {
            ...
            console.warn('invalid response from google ', { err, response })
            if (response && response.data && response.data.files) console.warn('files', response.data.files)
    ...
    function (err) {
      if (err) {
        console.warn('error in reading recursive directory for ' + path, err)
        callback(err)
    ...
    if (err) {
      ...
      async.doWhilst(function (cb) {
        self.drive.files.list({
          ...
          if (err) {
            console.warn('getnamesForAppendFilesFrom err for path:' + appendDirectory, { res, err })
            cb(err)
          } else if (!res.data || !res.data.files) {
            console.warn('getnamesForAppendFilesFrom err for path:' + appendDirectory, { res, err })
            ...
            res.data.files.forEach(function (file) {
              // // console.log('Found file: ', file.name, file.id, file.createdTime)
              list.push(file)
            })
            pageToken = res.nextPageToken
            cb()
        ...
        // // console.log('allentries.len :' + (list ? list.length : '0'))
        const entries = []
...
export default googleDriveFS
export { googleDriveFS as cloudFS }
