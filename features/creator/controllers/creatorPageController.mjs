...
// Serves the ask-app builder page (/creator/ask). Same creator app + token as the full builder,
// but a lighter chat-only UI. See freezr_askapps_plan_v1.md §5.
const generateAskBuilderPage = async (req, res) => {
  try {
    const appFS = res.locals.freezr?.appFS
    if (!appFS) {
      console.error('appFS not found for ask builder page')
      return res.status(500).send('Internal server error - appFS not available')
    }

    const options = {
      page_title: 'Ask your apps',
      css_files: ['/app/info.freezr.public/public/freezr_style.css', 'ask.css'],
      page_url: 'ask.html',
      app_name: 'info.freezr.creator',
      script_files: [],
      modules: ['ask.js'],
      ...
      user_is_admin: req.session.logged_in_as_admin,
      user_is_publisher: req.session.logged_in_as_publisher,
      other_variables: 'const thisPage = "ask";',
      sdkAddons: sdkAddonsForApp('info.freezr.creator', null)
    }
    ...
    console.error('Error generating ask builder page:', error)
    return res.status(500).send('Internal server error')
  }
}

export const createCreatorPageController = () => ({ generateCreatorPage, generateAskBuilderPage })
