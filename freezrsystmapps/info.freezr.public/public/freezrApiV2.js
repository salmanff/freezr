// freezrApiV2.js - Modern Freezr API (core)
// Version 2.0.0 - 2026
//
// SDK add-ons live in sibling files and are injected only when an app needs
// them (driven by manifest permissions + common/systemPermissions.json):
//   - freezrApiV2.connections.js — attaches freezr.connections (use_mail today,
//     use_contacts / use_calendar in future)
//   - freezrApiV2.llm.js          — attaches freezr.llm (use_llm)
//   - freezrApiV2.serverless.js   — attaches freezr.serverless (use_serverless
//     and/or use_3pFunction)
// The page renderer (adapters/rendering/pageLoader.mjs) appends the right
// <script> tags after this core file but before app-level scripts, so app code
// can safely use freezr.connections.* / freezr.llm.* when permitted.
...
const freezr = (function() {
  ...
  async function apiRequest(method, path, body = null, options = {}) {
    ...
    // Get access token
    // Federation endpoints (transfer_info/transfer_proceed on the provider side,
    // store_transferred_credentials on the consumer side) authenticate via the
    // one-time `state` param rather than an app token, and are called from public
    // pages where no app token is available.
    const PATHS_WITHOUT_TOKEN = [
      ...
      '/oauth/transfer_info', '/oauth/transfer_proceed', '/oauth/store_transferred_credentials'
    ]
    let pathOnly;
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Preserve the full structured error body so callers can read fields like
        // reauth_url / connectionName when the server returns e.g.
        // { error: 'token_expired', connectionName, reauth_url } on a 401.
        // The bare Error message only captures errorData.error/.message above.
  ...
  const freezr = {
    ...
    perms: {
      ...
      async isGranted(permissionName, options = {}) {
        const url = '/ceps/perms/get' + (options.owner ? ('?owner=' + options.owner) : '')
    ...
    menu: {
      ...
      // A prominent, self-styled top banner for ERROR-severity notifications (e.g. "your storage
      // credentials are invalid"). The freezr core client loads from info.freezr.public — which is
      // served even when the user's own storage is unreachable — so this warning shows on every
      // page (incl. the home page) even when the page's own scripts/data could not load. Inline
      // styles only, so it doesn't depend on any per-app CSS that may also have failed to load.
      _renderErrorBanner() {
        try {
          const notifications = (freezrMeta && freezrMeta.notifications) || []
          const errors = notifications.filter(n => (n.severity === 'error'))
          if (!errors.length || document.getElementById('freezr_error_banner')) return
          const n = errors[0]
          const banner = document.createElement('div')
          banner.id = 'freezr_error_banner'
          banner.setAttribute('style', 'position:fixed; top:0; left:0; right:0; z-index:2147483646; background:#dc2626; color:#fff; padding:10px 16px; font-family:system-ui,sans-serif; font-size:14px; line-height:1.4; box-shadow:0 2px 6px rgba(0,0,0,0.25); display:flex; align-items:center; gap:12px;')
          const msg = document.createElement('div')
          msg.style.flex = '1'
          msg.innerHTML = '<strong>' + (n.title || 'Warning') + '</strong> &mdash; ' + (n.message || '')
          banner.appendChild(msg)
          if (n.action && n.action.url) {
            const a = document.createElement('a')
            a.href = n.action.url
            a.textContent = n.action.label || 'Fix'
            a.setAttribute('style', 'color:#dc2626; background:#fff; padding:5px 12px; border-radius:5px; text-decoration:none; font-weight:bold; white-space:nowrap;')
            banner.appendChild(a)
          }
          const close = document.createElement('div')
          close.textContent = '✕'
          close.setAttribute('style', 'cursor:pointer; padding:0 4px; opacity:0.9;')
          close.onclick = () => banner.remove()
          banner.appendChild(close)
          document.body.appendChild(banner)

          // Pages can mark content that is meaningless while resources are unreachable (e.g. the
          // home page's app list + install box, which can't load anyway) with this class; hide it
          // so the page isn't cluttered with broken/empty sections behind the warning.
          document.querySelectorAll('.freezr-hide-on-resource-error').forEach(el => { el.style.display = 'none' })
    ...
    // freezr.serverless.* lives in freezrApiV2.serverless.js, injected only
    // when the app's manifest declares use_serverless or use_3pFunction (or
    // when systemPermissions.json has a matching shortcut for the app).
    // See common/helpers/sdkAddons.mjs.
