...
export async function mountAllModernRoutes(app, { dsManager, freezrPrefs, freezrStatus, logManager }) {
  ...
  try {
    ...
    try {
      ...
      const { createMessagingApiRoutes } = await import('../features/connections/messaging/messagingRoutes.mjs')
      const { createFsApiRoutes } = await import('../features/connections/fs/fsRoutes.mjs')
      ...
      const messagingApiRoutes = createMessagingApiRoutes({ dsManager, freezrPrefs, freezrStatus })
      const fsApiRoutes = createFsApiRoutes({ dsManager, freezrPrefs })
      ...
      app.use('/feps/connections/messaging', messagingApiRoutes)
      app.use('/feps/connections/fs', fsApiRoutes)
      ...
      results.mounted.push('messaging-api')
      results.mounted.push('fs-api')
    } catch (error) {
